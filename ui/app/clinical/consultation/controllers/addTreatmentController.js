...
angular.module('bahmni.clinical')
    .controller('AddTreatmentController', ['$scope', '$rootScope', 'contextChangeHandler', 'treatmentConfig', 'drugService',
        ...
        function ($scope, $rootScope, contextChangeHandler, treatmentConfig, drugService, $timeout,
            ...
            $scope.add = function () {
                ...
                spinner.forPromise(drugService.getDrugStockStatus(
                    ...
                    if (!drugStatusResponse) {
                        console.log(drugStatusResponse);
                        $scope.stockAvaibilityStatus = !drugStatusResponse;
                        $scope.stockAvaibilityDrugOrder = newDrugOrder;
                        ngDialog.open({
                            template: 'consultation/views/treatmentSections/availableDrugOrderModal.html',
                            scope: $scope
                        });
                        return;
                    }
            ...
            $scope.drugStockAvaibilityContinue = function (newDrugOrder) {
                $scope.popupActive = false;
                ngDialog.close();
                $scope.clearForm();
                $scope.treatments.push(newDrugOrder);
            };

            $scope.drugStockAvaibilityCancel = function () {
                $scope.popupActive = false;
                ngDialog.close();
                $scope.clearForm();
            };
            ...
            var getConflictingDrugOrder = function (newDrugOrder) {
                ...
                var unsavedNotBeingEditedOrders = _.filter(allDrugOrders, {
                    isBeingEdited: false
                });
                var existingDrugOrders;
                if (newDrugOrder.isBeingEdited) {
                    existingDrugOrders = _.reject($scope.consultation.activeAndScheduledDrugOrders, {
            ...
            var contextChange = function () {
                ...
                if (isSameDrugBeingDiscontinuedAndOrdered()) {
                    return {
                        allow: false,
                        errorMessage: errorMessages.discontinuingAndOrderingSameDrug
                    };
                }
                ...
                if ($scope.unaddedDrugOrders()) {
                    return {
                        allow: false,
                        errorMessage: errorMessages.incompleteForm
                    };
                }
                return {
                    allow: true
